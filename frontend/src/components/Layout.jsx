...
function Sidebar({ currentPage, onNavigate }) {
  ...
  const renderNavItem = (item) => {
    ...
    const isActive = currentPage === item.id || window.location.pathname === item.path;
  ...
  return (
    <div
      className={`bg-gray-900 text-white h-screen flex flex-col transition-all ${
        collapsed ? 'w-16' : 'w-64'
      }`}
    >
      {/* Header */}
      ...
      <nav className="flex-1 p-2 space-y-1 overflow-y-auto">
        {/* Staff view: show staff section first */}
        {isStaff && nav.staff && (
          <>
            {!collapsed && (
            ...
            {nav.staff.map(renderNavItem)}

            {/* Personal section for staff */}
            {nav.personal && (
              <>
                {!collapsed && (
                ...
                {nav.personal.map(renderNavItem)}
              </>
            )}
          </>
        )}

        {/* Client view: show main nav */}
        {!isStaff && nav.main && nav.main.map(renderNavItem)}

        {/* Secondary nav */}
        {nav.secondary && nav.secondary.length > 0 && (
          <>
      ...
      {user && (
        <div className="p-4 border-t border-gray-800">
          <div className={`flex items-center gap-3 ${collapsed ? 'justify-center' : ''}`}>
            <div className="w-8 h-8 bg-amber-600 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0">
              {user.first_name?.[0]}
              {user.last_name?.[0]}
            </div>
            {!collapsed && (
...
export function Toast({ message, type = 'success', onClose }) {
  ...
  return (
    <div
      className={`fixed bottom-20 md:bottom-4 right-4 px-6 py-3 rounded-lg shadow-lg z-50 text-white ${
...
export function Button({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`font-medium rounded-lg transition disabled:opacity-50 ${variants[variant]} ${sizes[size]} ${className}`}
    >
      {children}
    </button>
  );
